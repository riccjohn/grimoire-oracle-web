...
Complete the ingestion pipeline that reads TTRPG markdown files from `vault/`, splits them into chunks, embeds them with Google's `gemini-embedding-001`, and writes them to Supabase pgvector — verifiable both locally and via a manual GitHub Actions job.
...
**Tasks:**
1. Install: `pnpm add @langchain/google-genai @langchain/community @supabase/supabase-js`
...
**Tasks:**
1. Import `GoogleGenerativeAIEmbeddings` from `@langchain/google-genai`
...
**Verification:**
- [ ] `pnpm ingest` runs without throwing
- [ ] Console shows chunk count at each stage
- [ ] Supabase Table Editor shows new rows in `documents` with non-null `embedding` column (3072 dimensions)

#### Agent Context
- **Files to modify:** `scripts/ingest.ts`
- **Test command:** `pnpm ingest`
- **Acceptance gate:** Script exits 0; rows visible in Supabase with 3072-dim embeddings
- **Constraints:**
  - Use `model: "gemini-embedding-001"` for embeddings
  - `SupabaseVectorStore.fromDocuments()` handles both embedding + upsert — do not call the embedding model separately
  - The Supabase `documents` table must already exist with `embedding vector(3072)` column before running
...
- **Supabase table:** The `documents` table with `embedding vector(3072)` must be created in the Supabase dashboard before Phase 3 can run
